...
-p 55${CONTAINER_ID}:5500                     `# Maps host_port:container_port for EM - use container port 5500` \
...
-v /oradata:/opt/oracle/oradata               `# Maps data volume host_volume:container volume - use container volume /opt/oracle/oradata - host_volume must be owned by 54321:54321` \
-v /oradiag:/opt/oracle/diag                  `# Maps log volume host_volume:container volume - use container volume /opt/oracle/diag - host_volume must be owned by 54321:54321` \
oracle/database/12.2.0.1                      `# Image Repository/tag`
